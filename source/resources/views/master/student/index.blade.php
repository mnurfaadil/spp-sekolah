                                    ...
                                    <div class="col-md-6">
                                        <div style="float:right;flex-direction:row;">
                                            <form target="_blank" action="{{route('pdf.print.rekap.siswa')}}" role="form" method="post">
                                                ...
                                                <a @php echo ($jml < 1 ? 'onclick="peringatanJurusan()"': $angkatans->count() ) < 1 ? 'onclick="peringatanAngkatan()"' : 'data-toggle="modal" href="#modalAdd"' @endphp class="btn btn-success" ><i class="fa fa-plus"></i> Tambah </a>
                            ...
                            <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                ...
                                @foreach($students as $d)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$d->nis}}</td>
                                        <td>{{$d->nama}}</td>
                                        <td><div style="text-align:center;">{{$d->jenis_kelamin}}</div></td>
                                        <td><div style="text-align:center;">{{$d->kelas}}</div></td>
                                        <td><div style="text-align:center;">{{$d->major->nama}}</div></td>
                                        <td><div style="text-align:center;">{{$d->angkatans->angkatan}} ({{$angkatans->tahun}})</div></td>
                                        <td><div style="text-align:center;">{{$d->alamat}}</div></td>
                                        <td><div style="text-align:center;">{{$d->phone}}</div></td>
                                        <td>
                                        <div style="text-align:center;">
                                          <a href="#" class="btn btn-info" onclick="detailConfirm({{$d}},'{{$date}}')"title="Detail">
                                            <i class="fa fa-eye"> Detail</i>
                                          </a>
                                        <a href="#" class="btn btn-warning"
                                            onclick="editConfirm({{$d}},'{{$date}}')"
                                            title="Edit"><i class="fa fa-edit"> Edit</i></a>
                                        <a href="{{ route('students.destroy',$d->id) }}" class="btn btn-danger"
                                            onclick="event.preventDefault();destroy('{{ route('students.destroy',$d->id) }}');"
