                        ...
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            ...
                            <table id="table" data-toggle="table" data-pagination="true" data-search="true"
                                ...
                                @foreach($datas as $siswa)
                                        @php
                                            $detail = intval($siswa->detail->count());
                                            $detail_nunggak = $siswa->detail->where('jenis_pembayaran','Nunggak')->count();
                                            $nominal = intval($siswa->periode->first()->nominal);
                                            $besaran = $detail*$nominal;
                                            $terbayar = intval(($siswa->detail->sum('nominal')!=0)?$siswa->detail->sum('nominal'):0);
                                            $sisa = $besaran - $terbayar;
                                            $detail_nunggak = $siswa->detail->where('status','Nunggak')->count();
                                        @endphp
                                    <tr>
                                        <td>{{$no++}}</td>
                                        <td>{{$siswa->student->nama}}</td>
                                        <td>{{$siswa->student->kelas}} - {{$siswa->student->major->nama}}</td>
                                        <td>
                                            <div style="text-align:right">
                                                {{number_format($besaran,0,',','.')}}
                                            </div>
                                        </td>
                                        <td>
                                            <div style="text-align:right">
                                        ...
                                        <td>
                                            <div style="text-align:center">
                                            @if($siswa->cekWaiting==0 && $detail_nunggak==0 && $sisa==0 && $besaran!=0)
                                        ...
                                        <td>
                                            <a href="{{ route('payment.monthly.show.detail',[$siswa->id, $siswa->student->id, $financing->id]) }}" class="btn btn-success"
